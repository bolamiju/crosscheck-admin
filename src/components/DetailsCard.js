...
const DetailsCard = ({ info, activeTab, setInfo }) => {
  ...
  const handleUpdateVerification = async () => {
    ...
    const response = await axios.put(
      `${BASE_URL}/api/v1/verifications/${info.id}/${info.email}`,
      formData,
      {
  ...
  const handleEmail = async (e) => {
    ...
    const response = await axios.post(
      `${BASE_URL}/api/v1/verifications/sendemail/${info.email}/${info.requester}`,
      { ...inputValues, id, name, institution }
    );
        ...
        {(activeTab === "pending" || activeTab === "processing") && (
          <div className="select">
            ...
            {verificationStatus === "completed" ? (
              <input type="file" name="proof" onChange={handleDocument} />
            ...
            {verificationStatus !== "email" && (
              <button onClick={handleUpdateVerification} className="finish">
            ...
            {verificationStatus === "email" && (
              <button onClick={handleEmail} className="finish">
